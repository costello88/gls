...
interface ApiDeleteLabelResponse {
  error?: boolean;
  status?: string;
  message?: string;
  errors?: unknown;
...
export async function deleteGlsLabel(unitNo: string): Promise<void> {
  const requestBody = {
  ...
  const response = await postGlsApi("/Label/Delete", requestBody);
  const json = response.json as ApiDeleteLabelResponse | null;
  ...
  if (json.error) {
    throw new GlsApiError(
      json.message ?? "GLS API reported an error deleting the label",
      response.httpStatus,
      json.status,
      json.errors,
    );
